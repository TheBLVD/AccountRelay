...
class Request
  ...
  def perform
    begin
      response = http_client.public_send(@verb, @url.to_s, @options.merge(headers:))
  ...
  class Socket < TCPSocket
    class << self
      ...
      def private_address_exceptions
        @private_address_exceptions = (ENV['ALLOWED_PRIVATE_ADDRESSES'] || '').split(',').map do |addr|
          IPAddr.new(addr)
        end
      end
    end
